...
async def status_task() -> None:
    ...
    """
    statuses = ["with you!", "with azuxi!", "with humans!"]
    ...
    if message.author.bot != True:
        try:
            ...
            if result[1]==99:
                ...
                await message.channel.send(f"{message.author.mention} advanced to level {result[2]+1}")
                cur.execute(f"UPDATE GUILD_{message.guild.id} SET xp=0, lvl={result[2]+1} WHERE user_id={message.author.id}")
                con.commit()
...
async def on_message_edit(before, after):
    channel = bot.get_channel(1019240602849771610)
